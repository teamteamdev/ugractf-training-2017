...
class Game:
    def __init__(self):
        self.balloon_lane = LANES_NUMBER // 2
        self.balloon_progress = 0
    ...
    def current_plane_generation_probability(self):
        ...
        distance_to_top_plane = CANVAS_HEIGHT - (last_plane_bottom - self.balloon_progress + PLANE_SPRITE.height - 1) - 1

        if distance_to_top_plane < 0:
            return 0, []
        if distance_to_top_plane < BALLOON_SPRITE.height - 1 and last_plane_lane not in (0, LANES_NUMBER - 1):
    ...
    def manage_planes(self):
        ...
        if random.random() < probability:
            self.planes.append((self.balloon_progress + CANVAS_HEIGHT, random.choice(suitable_lanes)))
        passed_planes_number = 0
        for bottom_position, _ in self.planes:
            if bottom_position - self.balloon_progress + PLANE_SPRITE.height - 1 >= 0:
                break
    ...
    def tick(self, action):
        ...
        if action == Action.MOVE_LEFT:
            self.balloon_lane = max(0, self.balloon_lane - 1)
        elif action == Action.MOVE_RIGHT:
            self.balloon_lane = min(LANES_NUMBER - 1, self.balloon_lane + 1)
        self.balloon_progress += 1
        self.manage_planes()
        balloon_range = (self.balloon_progress, self.balloon_progress + BALLOON_SPRITE.height - 1)
        for bottom_position, lane in self.planes:
            plane_range = (bottom_position,
                           bottom_position + PLANE_SPRITE.height - 1)
            if lane == self.balloon_lane and ranges_overlap(plane_range, balloon_range):
                self.game_over = True
                return
        if self.balloon_progress >= SHOW_FLAG_AFTER:
            self.draw_flag = True
    ...
    def draw_balloon(self, canvas):
        BALLOON_SPRITE.draw(
            canvas,
            (get_sprite_x_by_lane(BALLOON_SPRITE, self.balloon_lane),
             CANVAS_HEIGHT - BALLOON_SPRITE.height))
    ...
    def draw_plane(self, canvas, bottom_position, lane):
        ...
        plane_y = CANVAS_HEIGHT - (bottom_position - self.balloon_progress + PLANE_SPRITE.height)
        PLANE_SPRITE.draw(canvas, (plane_x, plane_y))

    def draw_score(self, canvas):
        score = str(self.balloon_progress)
